...
/**
 * Local card cache + offline action queue.
 *
 * All methods are synchronized. Do NOT close SQLiteDatabase from
 * SQLiteOpenHelper on every call — concurrent close() races crash on
 * Android 4.4 with "connection pool has been closed".
 */
public class CardRepository {
    ...
    public CardRepository(Context context) {
        dbHelper = new InkBriefDatabase(context.getApplicationContext());
    }

    public synchronized List<Card> getAllCards(String date) {
        ...
        try {
            ...
            if (cards.isEmpty()) {
                ...
                String latest = queryLatestDeckDate(db);
    ...
    public synchronized String getLatestDeckDate() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        return queryLatestDeckDate(db);
    }

    private String queryLatestDeckDate(SQLiteDatabase db) {
        Cursor d = null;
        try {
            d = db.rawQuery(
    ...
    public synchronized Card getCard(String id) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Card card = null;
        Cursor cursor = null;
        try {
    ...
    public synchronized void insertOrUpdate(List<Card> cards) {
        ...
        try {
            ...
            if (deckDate == null || deckDate.length() == 0) {
                String latest = queryLatestDeckDate(db);
                deckDate = latest != null ? latest : "";
                for (Card c : cards) {
                    c.setDate(deckDate);
                }
            }

            HashMap<String, String> localFeedback = new HashMap<String, String>();
            ...
            for (Card card : cards) {
                ...
                if (keep != null) {
                    // Keep local liked/skipped row; do not overwrite.
                    continue;
                }
    ...
    public synchronized void updateStatus(String cardId, String status) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
    ...
    public synchronized void enqueueAction(String cardId, String action) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
    ...
    public synchronized void removePendingAction(String cardId, String action) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
    ...
    public synchronized void bumpPendingTries(String cardId, String action) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        db.execSQL(
